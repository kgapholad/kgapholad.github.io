            ...
            <div class="card">
				...
				<table class="table"><thead><th>Part1</th><th>Low</th><th>High</th><th>Route</th><th>From</th><th>To</th><th>Action</th></thead>
				 ...
				 <!-- @foreach ($ussds as $ussd)
						...
						<td><a  class="nav-link" href="{{ url('/ussds/'.$ussd->Routing_Num ) }}">view</a></td>
					</tr>
					@endforeach-->
					</table>
					<ul class="page-link"><b>Total [ {{$ussds->count()}}	]	</b>			
						<div style="text-align:center">
							<!---------------- Prev --------->
							@if ($page == 1 ||$page == 0)
           						<!-- <a class="pagination-previous" disabled>Previous</a>-->
								@else
									<a  class="previous round" href="{{url('/ussds?page=1') }}" rel="prev">&#8249;&#8249;</a>
            						<a  class="previous round" href="{{url('/ussds?page='.$page-=1) }}" rel="prev">&#8249; </a>
							@endif
							<!-------------- End Prev --------->
							@for ($i = 1; $i <= ceil(count($ussds) /$NUM_PER_PAGE); $i++) 
							<?php
								if(isset($_GET['page'])){
							...
							?>
									...
									@if ($i == $_GET['page'])	
									<a class="round"  href="{{ url('/ussds?page='.$i ) }}">[<i>{{$i}}</i> ]</a>
									@else
									@if ($i == $Pshow || $i == $Nshow)
									<a  class="round" href="{{ url('/ussds?page='.$i ) }}">{{$i}}</a>
									@endif
									@endif	
							 ...
							 ?>
								 <a class="next round" href="{{ url('/ussds?page='.$nxt) }}" rel="next">&#8250;</a>
								 <a class="next round" href="{{ url('/ussds?page='.$lst) }}" rel="next">&#8250;&#8250;</a>
								 <?php 
							 }/*else
								 { ?>
